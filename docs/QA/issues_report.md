...
<BalanceCard token="Real Brasileiro" amount={...} icon={...} isLoading={isLoading} />
<BalanceCard token="Bitcoin" amount={...} icon={...} isLoading={isLoading} />
<BalanceCard token="Ethereum" amount={...} icon={...} isLoading={isLoading} />
```
...
**Hipótese secundária — Zod validation loop:**
O `useEffect` de cotação no `SwapDrawer` (não aplicável aqui diretamente, mas o padrão de `watch()` pode re-renderizar com valor inválido).

**Hipótese terciária — `amount || 0` na description:**
Quando `amount` é `undefined` (estado inicial do form), `amount || 0` passa `0`, mas quando o usuário está digitando e o valor é, por ex. `"12."`, a string `"12."` é passada diretamente para `formatToken`, que tenta `new Decimal("12.")` — isso é válido no Decimal.js, mas pode gerar comportamento inesperado dependendo da versão.
...
|---|-------|------------|--------|
| 1 | Saldo disponível não exibido no Saque | 🔴 Alta | WithdrawDrawer |
| 2 | Remover valores convertidos em BRL dos cards | 🟡 Média | DashboardPage, BalanceCard |
| 3 | Página em branco ao digitar no campo de valor | 🔴 Crítica | WithdrawDrawer, formatters |
| 4 | BRL card exibe valor duplicado | 🟡 Média | DashboardPage |
| 5 | Drawer sem animação de entrada/saída | 🟢 Baixa | Drawer |
| 6 | formatToken com 8 decimais em todos os tokens | 🟡 Média | formatters |
| 7 | formatDate sem tratamento de erro | 🟡 Média | formatters |
| 8 | Token JWT em localStorage (XSS vuln.) | 🟠 Alta (Seg.) | AuthContext |
| 9 | Ausência de Error Boundary global | 🔴 Crítica | App |
| 10 | Conflito de tokens no SwapDrawer | 🟢 Baixa | SwapDrawer |
| 11 | Acesso ao Histórico redireciona p/ login (401) | 🔴 Crítica | HistoryDrawer / Backend |
